...
export const GameProvider = ({ children }) => {
  ...
  const [longerLastingCellsEnabled, setLongerLastingCellsEnabled] = useState(false);
  ...
  const updateGrid = useCallback(() => {
    ...
    for (let row = 0; row < gridSize.rows; row++) {
      for (let col = 0; col < gridSize.cols; col++) {
        ...
        if (longerLastingCellsEnabled && !cellShouldLive && !newMovingCells.has(`${row},${col}`)) {
  ...
  function tryMoveCell(row, col, newGrid, gridSize) {
    ...
    for (let { dr, dc } of directions) {
      ...
      if (newRow >= 0 && newRow < gridSize.rows && newCol >= 0 && newCol < gridSize.cols && !newGrid[newRow][newCol]) {
